...
#define maxTextureSize      uint(u_TextureSize.x)
...
ivec2 calculateTextureCoord(uint index, uint size) {
    uint Y = index / size;
    uint X = index - (Y * size);
    return ivec2(X, Y);
}
...
void main() {
    vec4 tex0        = imageLoad(textureBuffer, calculateTextureCoord((gl_InstanceID * 4), maxTextureSize));
    vec4 tex1        = imageLoad(textureBuffer, calculateTextureCoord((gl_InstanceID * 4) + 1, maxTextureSize));
    vec4 tex2        = imageLoad(textureBuffer, calculateTextureCoord((gl_InstanceID * 4) + 2, maxTextureSize));
    vec4 tex3        = imageLoad(textureBuffer, calculateTextureCoord((gl_InstanceID * 4) + 3, maxTextureSize));

    vec3 p0          = tex0.xyz;
    vec3 p1          = tex1.xyz;
    vec3 normal0     = tex2.xyz;
    vec3 normal1     = tex3.xyz;
    vec4 color0      = uintABGRToVec4Color(floatBitsToUint(tex0.w));
    vec4 color1      = uintABGRToVec4Color(floatBitsToUint(tex1.w));
    vec4 lineColor   = uintABGRToVec4Color(floatBitsToUint(tex2.w));
